...
(function ($) {
            ...
            $self.on({
                "rupTable_serializeGridData.filter": function (events, postData) {
                    ...
                    if (lastFilterParams === undefined || lastFilterParams !== queryStrFilterParams) {
                        jQuery.extend(postData, {
                            page: ($self.data("tmp.firstLoad") === true && settings.core.startOnPage !== null ? settings.core.startOnPage : "1")
                        });
    ...
    jQuery.fn.rup_table("extend", {
         ...
         * @function
         * @name filter
         * @param  {boolean} async - Indica si la llamada debe realizarse en modo asíncrono (true) o síncrono (false).
         * @example
     ...
     * @name options
     * @param {boolean} [showHidden] - Determina si el formulario de filtrado se debe de mostrar inicialmente oculto o no
     * @param {string} url - Determina si el formulario de filtrado se debe de mostrar inicialmente oculto o no
     * @param {object} transitionConfig - Configuración del efecto de la animación de mostrar/ocultar el formulario de filtrado.
     * @param {function} fncSearchCriteria - Permite especificar una función de callback en la cual es posible modificar la cadena de texto con la que se muestra el resumen de los parámetros de filtrado.
     * @param {array} 	excludeSummary - Permite excluir del resumen del filtro los campos especificados. Se debe usar el name del elemento a evitar
     */
     ...
     * @event module:rup_table/filter#rupTable_beforeFilter
     ...
     * @event module:rup_table/filter#rupTable_filter_beforeClean
     * @property {Event} e - Objeto Event correspondiente al evento disparado.
     * @example
