...
Ãšnicamente se requiere la inclusión de los ficheros que implementan el componente (js y css) comentados en los apartados *Ficheros* y *Dependencias*.
...
* Devuelve los identificadores de todos los elementos seleccionados:
    ...
    ```
* Devuelve todos los datos seleccionados y que est�n cargados (en la p�gina actual):
    ```javascript
    $("#idTable").rup_table("getSelectedRows");
    ```
* Devuelve los identificadores de todos los elementos seleccionadas, adem�s de la l�nea y p�gina en la que est�n:
    ```javascript
...
Cabe decir que todos los plugins est�n montados sobre el contexto de la tabla, por ese motivo, dentro del contexto puedes acceder a todas sus propiedades y a todos sus plugins, por ejemplo:
```javascript
...
* **blockPKeditForm**: si deseas que el campo que contiene la clave primaria est� bloqueado en modo edici�n (`true` o `false`).
* **searchPaginator**: si deseas tener paginador con n�mero o no (`true` o `false`).
* **defaultOrder**: especifica que la tabla tome la primera columna como criterio de ordenaci�n por defecto (valor por defecto: `true`).      
...
```
Permite habilitar el uso de formularios din�micos aunque hay que hacer algunos cambios en las JSPs de edici�n. En [este documento](./rup.table.editForm.md#propiedades-de-configuraci�n) puede encontrarse m�s informaci�n al respecto.
&nbsp;
