...
![Imagen 1](img/rup.jqtable_1.png)
...
- Fichero de plugin: rup.jqtable-x.y.z.js
- Ruta theme: rup/css/
- Fichero de estilos: +theme.jqtable-x.y.z.css+ (modificable por el desarrollador) y ui.jqgrid.css (fichero base de los estilos de la tabla).
...
En el componente jqtable se ha optado por minimizar el código HTML que se genera al vuelo mediantejavascript. Esto permite una serie de mejoras.
...
Para simplificar la nomenclatura nodos los identificadores de los objetos HTML se derivan a partir del identificador base del componente jqtable.
...
Para el ejemplo supongamos que el componente RUP jqtable se invoca sobre el elemento base con identificador jqtable.
...
```xml
<div id="jqtable_div" class="rup-jqtable-container">
  <div id="jqtable_feedback"></div>
  <div id="jqtable_toolbar"></div>
  <div id="jqtable_filter_div" class="rup-jqtable-filter">
    <form id="jqtable_filter_form">
      <div id="jqtable_filter_toolbar" class="formulario_legend"></div>
      <fieldset id="jqtable_filter_fieldset" class="rup-jqtable-filter-fieldset">
        <!-- Campos del formulario de detalle -->
        <div id="jqtable_filter_buttonSet" class="right_buttons">          
	    	<button id="jqtable_filter_cleanButton" type="button" class="btn btn-warning rup-limpiar">
	        	<i class="mdi mdi-eraser"></i>
			    <span>
			    	<spring:message code="clear" />
			    </span>
			</button>
			<button id="jqtable_filter_filterButton" type="button" class="btn rup-filtrar rup-filter-dropdown">
  ...
  <div id="jqtable_grid_div">
    <!-- Tabla -->
    <table id="jqtable"></table>
    <!-- Barra de paginación -->
    <div id="jqtable_pager"></div>
  </div>
</div>
...
* **jqtable_div**: Capa contenedora del componente jqtable. Contendrá todos los módulos del mismo excepto el formulario de detalle.
* **jqtable_feedback**: Elemento sobre el que se creará el feedback.
* **jqtable_toolbar:** Elemento sobre el que se creará la botonera que contendrá las acciones a realizar sobre los registros.
* **jqtable_filter_div**: Capa que contiene el formulario de filtrado.
* **jqtable_filter_form**: Formulario de filtrado. Los campos incluidos en este formulario se utilizarán como valores de filtrado de los registros.
* **jqtable_filter_toolbar**: Capa que contendrá los controles de operación del fomulario de filrado (plegar,desplegar, resumen de criterios de filtrado,..)
* **jqtable_filter_fieldset**: Fieldset que contendrá los campos del formulario de filtrado.
* **filter_buttonSet**: Botonera del formulario de filtrado.
* **jqtable_filter_filterButton**: Botón que realiza el filtrado de los registros de la tabla.
* **jqtable_filter_cleanLink**: Enlace que realiza la limpieza de los campos del formulario.
* **jqtable_grid_div**: Capa que contiene la tabla propiamente dicha.
* **jqtable**: Componente HTML sobre el que se inicializa el componente RUP jqtable.
* **jqtable_pager**: Paginador de la tabla.

En caso de querer utilizar la edición en formulario se deberá de incluir en la misma jsp el siguiente código HTML:

```xml
<div id="jqtable_detail_div" class="rup-jqtable-formEdit-detail">
  <div id ="jqtable_detail_navigation"></div>
  <div class="ui-dialog-content ui-widget-content" >
    <form id="jqtable_detail_form">
      <div id ="jqtable_detail_feedback"></div>
      <!-- Campos del formulario de detalle -->
    </form>
  </div>
  <div class="rup-jqtable-buttonpane ui-widget-content ui-helper-clearfix">
    <div class="ui-dialog-buttonset rup_jqtableEdit_buttonsContainerResposive">
		<button id="jqtable_detail_button_save" class="btn btn-outline-primary rup_jqtableEdit_buttonsResposive fix-editForm-buttons-align" type="button">
			<spring:message code="save" />
		</button>
		<button id="jqtable_detail_button_save_repeat" class="btn btn-outline-primary rup_jqtableEdit_buttonsResposive fix-editForm-buttons-align" type="button">
			<spring:message code="saveAndContinue" />
		</button>
		<button id="jqtable_detail_button_cancel" class="btn btn-outline-primary rup_jqtableEdit_buttonsResposive" type="button">
...
* **jqtable_detail_div**: Capa contenedora del formulario de detalle.
* **jqtable_detail_navigation**: Capa donde se incluirán los controles de navegación y información de los registros.
* **jqtable_detail_form**: Formulario que contendrá los campos que permitirán la edición de los registros.
* **jqtable_detail_feedback**: Feedback del formulario de detalle.
* **jqtable_detail_button_save**: Botón de guardado del registro.
* **jqtable_detail_button_save_repeat**: Botón que permite guarder el registro y continuar el proceso de edición.
* **jqtable_detail_link_cancel**: Enlace que permite cancelar el proceso de edición.
...
El componente jqtable se ha implementado siguiendo una arquitectura modular. De este modo se consigue:
...
El componente *tabla* se presenta con una apariencia visual definida en el fichero de estilos **theme.rup.jqtable-x.y.z.css**.
...
Estos son los métodos generados en el Controller para gestionar las peticiones de las diferentes operaciones del componente jqtable:
