...
*/
    DataTable.masterDetail.init = function ( dt ) {
        ...
        if ($('input:hidden [name="' + ctx.oInit.masterDetail.masterPrimaryKey + '"]').length===0){
            const msgError = 'Se ha producido un error a la hora de inicializar la rup_table de detalle, no se ha incluído en el formulario de filtrado"#' + ctx.oInit.$filterForm.attr('id') + '" el input hidden de la clave del padre "' + ctx.oInit.masterDetail.masterPrimaryKey + '".';
            $.rup_messages('msgError', {
                title: 'Error en la inicialización de la tabla',
                message: msgError
            });
            throw msgError;
        }
